...
export default function Serves() {
  const services = [
    ...
    {
      icon: <FaNetworkWired size={40} className="text-[#01f3f4]" />,
      titel: "IT Specialist",
      descrption:
    ...
    {
      icon: <IoPulseOutline size={40} className="text-[#01f3f4]" />,
      titel: "SEO",
      descrption:
      ...
      <AnimatedHeader
        Children={
          <div id="services" className="text-white/80 py-16">
            ...
            >
              My <span className="text-[#01f3f4]">Services</span>
            </h1>
            <div className="grid grid-cols-1 container mx-auto p-3 sm:grid-cols-2 md:grid-cols-3 gap-5">
              {services.map((service, index) => (
                <div
                  key={index}
                  data-aos="flip-right"
                  data-aos-delay={2 * index}
                  className="bg-[#323846] rounded-md p-4 text-center"
                >
                  <div
                    data-aos="fade-up"
                    className="flex justify-center my-4 text-[#01f3f4]"
                  >
                    {service.icon}
                  </div>
                  <h2 className="text-xl font-bold">{service.titel}</h2>
                  <p>{service.descrption}</p>
                  <button className="bg-[#1daaaa] hover:bg-[#01f4f4ab] transition-colors duration-300 rounded-md mt-4 px-4 py-2">
                    Read More
                  </button>
                </div>
              ))}
